...
import {
  ...
  type MatchTransactionStatus,
  type TabletopCommandContext,
} from '../observability/matchTrace'
import {
...
type ReplaceGameOptions = {
  transactions?: MatchTransaction[]
}

type GameStore = GameState & {
  undoStack: GameState[]
  matchTransactions: MatchTransaction[]
  dispatch: (action: GameAction, context?: TabletopCommandContext) => void
  dispatchMany: (
    actions: GameAction[],
  ...
  getMatchTransactions: () => readonly MatchTransaction[]
  exportMatchLog: () => string
  clearMatchTransactions: () => void
  replaceGame: (game: GameState, options?: ReplaceGameOptions) => void
}

let actionSequence = 0
let transactionSequence = 0

type ActiveMatchTransaction = {
  id: string
  startedAt: number
  source: import('../types/game').GameActionSource
}

let activeMatchTransaction: ActiveMatchTransaction | undefined

const uniqueRuntimeId = (prefix: string, sequence: number): string =>
  globalThis.crypto?.randomUUID?.() ?? `${prefix}-${Date.now()}-${sequence}`

const createActionMetadata = (): ActionMetadata => ({
  actionId: uniqueRuntimeId('action', ++actionSequence),
  timestamp: Date.now(),
  transactionId: activeMatchTransaction?.id,
  source: activeMatchTransaction?.source ?? 'ENGINE',
})
...
const legalActivationVariableValues = (
  ...
  const manaUpperBound = Object.values(
    playerManaPool(state, sourceControllerId),
...
const toGameState = (state: GameStore): GameState => {
  ...
  return {
    ...
    autoAdvanceBeginningOfTurnPaused:
      gameState.autoAdvanceBeginningOfTurnPaused ?? false,
    failedDrawFromEmptyLibraryByPlayer:
      gameState.failedDrawFromEmptyLibraryByPlayer ?? {},
    delayedEffects: gameState.delayedEffects ?? [],
    linkedObjectGroups: gameState.linkedObjectGroups ?? [],
    temporaryContinuousEffects: gameState.temporaryContinuousEffects ?? [],
    temporaryGrantedTriggeredAbilities:
      gameState.temporaryGrantedTriggeredAbilities ?? [],
    temporaryCharacteristicEffects:
      gameState.temporaryCharacteristicEffects ?? [],
    temporaryProtectionEffects: gameState.temporaryProtectionEffects ?? [],
    damagePreventionEffects: gameState.damagePreventionEffects ?? [],
    playerRuleEffects: gameState.playerRuleEffects ?? [],
    airbendPermissions: gameState.airbendPermissions ?? [],
    restrictedMana: gameState.restrictedMana ?? [],
    untapRestrictions: gameState.untapRestrictions ?? [],
    typeContinuousEffects: gameState.typeContinuousEffects ?? [],
    playerControlEffects: gameState.playerControlEffects ?? [],
    attachmentControlEffects: gameState.attachmentControlEffects ?? [],
    copyContinuousEffects: gameState.copyContinuousEffects ?? [],
    replacementEffects: gameState.replacementEffects ?? [],
    perTurnEventMarkers: gameState.perTurnEventMarkers ?? {},
    extraTurnsQueued: gameState.extraTurnsQueued ?? 0,
    extraTurnQueue: gameState.extraTurnQueue ?? [],
    pendingCombatPhaseSkipsByPlayer:
      gameState.pendingCombatPhaseSkipsByPlayer ?? {},
    spellsCastThisTurn: gameState.spellsCastThisTurn ?? 0,
    spellsCastThisTurnByPlayer: gameState.spellsCastThisTurnByPlayer ?? {},
    spellCastHistoryThisTurn: gameState.spellCastHistoryThisTurn ?? [],
    cardsDrawnThisTurnByPlayer: gameState.cardsDrawnThisTurnByPlayer ?? {},
    permanentsEnteredThisTurn: gameState.permanentsEnteredThisTurn ?? [],
    creaturesAttackedThisTurnByPlayer:
      gameState.creaturesAttackedThisTurnByPlayer ?? {},
    triggeredAbilityTurnMarkers: gameState.triggeredAbilityTurnMarkers ?? {},
    temporaryBlockingRestrictions:
      gameState.temporaryBlockingRestrictions ?? [],
    attackRequirements: gameState.attackRequirements ?? [],
    blockRequirements: gameState.blockRequirements ?? [],
  }
}

type MatchTransactionDraft = ActiveMatchTransaction & {
  actorPlayerId: import('../types/player').PlayerId
  commandType: string
  command?: ParsedCommand
  context: TabletopCommandContext
  before: ReturnType<typeof summarizeGameState>
}

type MatchTransactionOutcome = {
...
const actionActorPlayerId = (
  state: GameState,
  action: GameAction,
): import('../types/player').PlayerId => {
  const candidate = action as GameAction & {
    actorPlayerId?: import('../types/player').PlayerId
    playerId?: import('../types/player').PlayerId
  }
  return candidate.actorPlayerId ?? candidate.playerId ?? state.activePlayerId
}

const beginMatchTransaction = (
  state: GameState,
  commandType: string,
  context: TabletopCommandContext = {},
  command?: ParsedCommand,
): MatchTransactionDraft => {
  const draft: MatchTransactionDraft = {
    id: uniqueRuntimeId('transaction', ++transactionSequence),
    startedAt: Date.now(),
    ...
    command,
    context,
    before: summarizeGameState(state),
  }
  activeMatchTransaction = draft
  return draft
}

const finishMatchTransaction = (
  draft: MatchTransactionDraft,
  after: GameState,
  outcome: MatchTransactionOutcome,
): MatchTransaction => {
  if (activeMatchTransaction?.id === draft.id)
    activeMatchTransaction = undefined
  const actions = after.history
    .filter((entry) => entry.transactionId === draft.id)
    .reverse()
    .map((entry) => ({
  ...
  return {
    ...
    rawInput: draft.context.rawInput,
    normalizedInput: draft.context.normalizedInput,
    provider: draft.context.provider,
    confidence: draft.context.confidence,
    status: outcome.status,
    description: outcome.description,
    error: outcome.error,
    implicitResolutions: [...(outcome.implicitResolutions ?? [])],
    actions,
    before: draft.before,
    after: summarizeGameState(after),
  }
}

const appendMatchTransaction = (transaction: MatchTransaction): void => {
  useGameStore.setState((state) => ({
    matchTransactions: [...state.matchTransactions, transaction].slice(
      -MAX_RECOVERY_TRANSACTIONS,
    ),
  }))
  const state = useGameStore.getState()
  saveMatchRecoverySnapshot(toGameState(state), state.matchTransactions)
}

const finalizeMutationTrace = (
  draft: MatchTransactionDraft,
  outcome: MatchTransactionOutcome,
): void => {
  const after = toGameState(useGameStore.getState())
  appendMatchTransaction(finishMatchTransaction(draft, after, outcome))
}

const abortMutationTrace = (
  draft: MatchTransactionDraft,
  error: unknown,
): void => {
  const message = error instanceof Error ? error.message : String(error)
  finalizeMutationTrace(draft, {
    status: 'ERROR',
    error: { code: 'RUNTIME_ERROR', message },
  })
}

const outcomeForTabletopExecution = (
  execution: TabletopExecutionResult,
): MatchTransactionOutcome => {
  switch (execution.status) {
    case 'error':
      return {
        status: 'ERROR',
        error: execution.error,
        implicitResolutions: execution.implicitResolutions,
      }
    case 'paused':
      return {
        status: 'PAUSED',
    ...
    case 'undo':
      return { status: 'UNDO', description: execution.description }
    case 'executed':
      return {
        status: 'EXECUTED',
        description: execution.description,
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  getMatchTransactions: () => get().matchTransactions,
  exportMatchLog: () =>
    serializeMatchLog(toGameState(get()), get().matchTransactions),
  clearMatchTransactions: () => set({ matchTransactions: [] }),
  dispatch: (action, context) => {
    const before = toGameState(get())
    const trace = beginMatchTransaction(before, action.type, {
      source: context?.source ?? 'UI',
      ...context,
      actorPlayerId:
        context?.actorPlayerId ?? actionActorPlayerId(before, action),
    })
    try {
      set((state) => {
        const previous = toGameState(state)
      ...
      finalizeMutationTrace(trace, {
        status: 'MUTATION',
        description: action.type,
      })
    } catch (error) {
      abortMutationTrace(trace, error)
      throw error
    }
  },
  dispatchMany: (actions, context) => {
    if (!actions.length) return
    const before = toGameState(get())
    const trace = beginMatchTransaction(
      before,
      `BATCH:${actions.map((action) => action.type).join('+')}`,
      {
        source: context?.source ?? 'UI',
        ...context,
        actorPlayerId:
          context?.actorPlayerId ?? actionActorPlayerId(before, actions[0]),
      },
    )
    try {
      set((state) => {
        const previous = toGameState(state)
        const containsCast = actions.some(
          (action) => action.type === 'CAST_SPELL',
        )
        const containsOrchestratedAction = actions.some(
          (action) =>
            action.type === 'NEXT_TURN' ||
            action.type === 'START_TURN' ||
            action.type === 'ACTIVATE_ABILITY' ||
            action.type === 'RESOLVE_SPELL',
        )
        const next =
          containsCast && !containsOrchestratedAction
            ? applyActionsWithEvents(previous, actions, 'DECLARATION')
            : actions.reduce((current, action) => {
                if (action.type === 'NEXT_TURN')
                  return advanceTurnPassUntilPlayerInteraction({
                    state: current,
                ...
                if (action.type === 'ACTIVATE_ABILITY')
                  return applyActivateAbilityAction(current, action)
                if (action.type === 'RESOLVE_SPELL')
                  return resolveSpellWithEffects(current, action)
                return applyActionsWithEvents(current, [action])
              }, previous)
        return next === previous
          ? state
          : { ...next, undoStack: [...state.undoStack, previous] }
      ...
      finalizeMutationTrace(trace, {
        status: 'MUTATION',
        description: `${actions.length} domain action(s)`,
      })
    } catch (error) {
      abortMutationTrace(trace, error)
      throw error
  ...
  declareExternalSpell: (card, manaSpent) => {
    const before = toGameState(get())
    const opponentId =
      opponentPlayerIds(before, before.localPlayerId ?? 'player-1')[0] ??
      'player-2'
    const trace = beginMatchTransaction(before, 'DECLARE_EXTERNAL_SPELL', {
      source: 'UI',
      rawInput: card.name,
    ...
    try {
      set((state) => {
        ...
        const opponentId =
          opponentPlayerIds(
            previous,
            previous.localPlayerId ?? 'player-1',
          )[0] ?? 'player-2'
      ...
      finalizeMutationTrace(trace, {
        status: 'MUTATION',
        description: `Declarar hechizo externo: ${card.name}`,
      })
    } catch (error) {
      abortMutationTrace(trace, error)
      throw error
  ...
  executeTabletopCommand: (command, context) => {
    const trace = beginMatchTransaction(
      toGameState(get()),
      command.type,
      context,
    ...
    try {
      set((state) => {
        ...
        const applyCommandActions = (actions: GameAction[]): GameState => {
          if (actions.some((action) => action.type === 'CAST_SPELL'))
            return applyActionsWithEvents(current, actions, 'DECLARATION')
        ...
        for (let attempts = 0; attempts < 48; attempts += 1) {
          const wantsCombatProgress =
            combatProgressCommand(command) ||
            commandImpliesPostcombatMain(command)
          ...
          if (
            ...
            if (
              ...
              implicitResolutions.push(
                combatProgressLabel(current.turnState.step),
              )
              combatAdvanced = true
            } else if (
              command.type === 'DECLARE_BLOCKERS' &&
              ...
              if (
                ...
                current = applyActionsWithEvents(current, [
                  {
                    ...
                    blockers: [],
                    actorPlayerId: opponentPlayerIds(
                      current,
                      activePlayerIdOf(current),
                    )[0],
                  },
                ])
                ...
                if (plan.type === 'DECISION') {
                  ...
                  execution = {
                    status: 'paused',
                    description:
                      'El combate necesita una asignación de daño antes de continuar.',
                    implicitResolutions,
                  }
                  return {
                    ...current,
            ...
            if (combatAdvanced) {
              if (
                ...
                execution = {
                  status: 'paused',
                  description:
                    'El combate avanzó hasta una decisión que necesita al jugador.',
                  implicitResolutions,
                }
                return { ...current, undoStack: [...state.undoStack, previous] }
              }
          ...
          if (
            current.stackResolutionMode === 'STRICT' &&
            current.stack.length > 0 &&
            command.type !== 'RESOLVE_SPELL' &&
            ...
            execution = {
              ...
              error: {
                ...
                message: 'Resuelve el stack antes de declarar otra acción.',
              },
              implicitResolutions,
            }
            return current === previous
          ...
          if (resolved.status === 'resolved') {
            if (command.type === 'ADVANCE_STEP' && command.targetStep) {
              const advance = advanceToStepUntilPlayerInteraction({
                state: current,
              ...
              execution = advance.stoppedForInteraction
                ? {
                    status: 'paused',
                    description:
                      advance.stopMessage ??
                      `El avance se detuvo en ${current.turnState.step}.`,
                    implicitResolutions,
                  }
                : {
                    status: 'executed',
            ...
            if (command.type === 'NEXT_TURN') {
              const setupActions = resolved.actions.filter(
                (action) => action.type !== 'NEXT_TURN',
              )
              current = applyCommandActions(setupActions)
              const pass = advanceTurnPassUntilPlayerInteraction({
                state: current,
                applyAction: (turnState, step) =>
                  applyActionsWithEvents(turnState, [step]),
              })
              ...
              if (
                pass.stop?.kind === 'RULE' &&
                pass.stop.code !== 'ATTACK_REQUIREMENT_NOT_MET'
              ) {
                execution = {
                  status: 'error',
                  error: {
                    code: pass.stop.code,
                    message: pass.stop.message,
                  },
                  implicitResolutions,
                }
              } else if (pass.stoppedForInteraction) {
                execution = {
                  status: 'paused',
                  description:
                    pass.stop?.message ??
                    'El avance del turno se detuvo porque hace falta una decisión.',
                  implicitResolutions,
                }
              } else {
                execution = {
    ...
    const finalExecution = execution as TabletopExecutionResult
    finalizeMutationTrace(trace, outcomeForTabletopExecution(finalExecution))
    return finalExecution
  },
  resolvePendingDecisionWithExternalCard: (decisionId, card, context) => {
    const before = toGameState(get())
    const decisionPlayerId = before.pendingDecisions.find(
      (decision) => decision.id === decisionId,
    )?.decisionPlayerId
    const trace = beginMatchTransaction(before, 'PENDING_EXTERNAL_CARD', {
      source: context?.source ?? 'UI',
      rawInput: context?.rawInput ?? card.name,
      ...context,
      actorPlayerId:
        context?.actorPlayerId ?? decisionPlayerId ?? before.activePlayerId,
    })
    try {
      set((state) => {
        const previous = toGameState(state)
      ...
      finalizeMutationTrace(trace, {
        status: 'MUTATION',
        description: `Resolver decisión ${decisionId} con ${card.name}`,
      })
    } catch (error) {
      abortMutationTrace(trace, error)
      throw error
      ...
      set((state) => {
        ...
        return previous
          ? {
              ...
              matchTransactions: state.matchTransactions,
            }
          : state
      })
      finalizeMutationTrace(trace, {
        status: 'UNDO',
        description: 'Restaurar snapshot anterior',
      })
    } catch (error) {
      abortMutationTrace(trace, error)
      throw error
  ...
  resolvePendingAbility: (pendingId, context) => {
    const trace = beginMatchTransaction(
      toGameState(get()),
    ...
    try {
      ...
      finalizeMutationTrace(trace, {
        status: 'MUTATION',
        description: `Resolver habilidad pendiente ${pendingId}`,
      })
    } catch (error) {
      abortMutationTrace(trace, error)
      throw error
  ...
  resolvePendingAbilityPayment: (pendingId, selection, context) => {
    const trace = beginMatchTransaction(
      toGameState(get()),
    ...
    try {
      ...
      finalizeMutationTrace(trace, {
        status: 'MUTATION',
        description: `${selection} ${pendingId}`,
      })
    } catch (error) {
      abortMutationTrace(trace, error)
      throw error
  ...
  ignorePendingAbility: (pendingId, context) => {
    const trace = beginMatchTransaction(
      toGameState(get()),
    ...
    try {
      ...
      finalizeMutationTrace(trace, {
        status: 'MUTATION',
        description: `Ignorar habilidad pendiente ${pendingId}`,
      })
    } catch (error) {
      abortMutationTrace(trace, error)
      throw error
  ...
  resolvePendingDecision: (decisionId, selection, context) => {
    const before = toGameState(get())
    const decisionPlayerId = before.pendingDecisions.find(
      (decision) => decision.id === decisionId,
    )?.decisionPlayerId
    const trace = beginMatchTransaction(before, 'PENDING_DECISION', {
      source: context?.source ?? 'UI',
      ...context,
      actorPlayerId:
        context?.actorPlayerId ?? decisionPlayerId ?? before.activePlayerId,
    ...
    try {
      set((state) => {
        ...
        if (decision?.type === 'CLEANUP_DISCARD_SELECTION') {
          ...
          const known = previous.cards.find(
            (card) =>
              card.zone === 'hand' &&
              card.instanceId === selection &&
              (card.ownerId ?? previous.localPlayerId ?? 'player-1') ===
                playerId,
          )
        ...
        if (decision?.type === 'CAST_GENERIC_CONTRIBUTION_SELECTION') {
          ...
          const contributionActions: GameAction[] = selected.map(
            (instanceId) =>
              contribution.contributionType === 'TAP_PERMANENTS'
            ...
            const reducedCost = {
              ...
              generic: Math.max(
                ...
                reductionContribution.cost.generic -
                  selected.length *
                    reductionContribution.genericReductionPerCard,
              ),
            }
            const planned = resolvePreparedCast(
              provisional,
        ...
        if (decision?.type === 'STACK_RETARGET_SELECTION') {
          ...
          if (
            ...
            return state
          const declaredTargets = retarget.declaredTargets.map(
            (target, index) =>
        ...
        if (decision?.type === 'HIDDEN_ZONE_CARD_SELECTION') {
          ...
          if (hidden) {
            if (selection === 'FAIL_TO_FIND') {
              ...
              if (step.type === 'ACTIONS')
                next = applyActionsWithEvents(next, [
                  ...step.actions,
                  {
                    type: 'REMOVE_PENDING_RESOLUTION',
                    resolutionId: resumed.id,
                  },
                  ...(step.resolution.completionActions ?? []),
                ])
              else if (step.type === 'COMPLETE' || step.type === 'ERROR')
            ...
            const declared = declaredHiddenZoneCard(
              previous,
              decision,
              selection,
            )
        ...
        const resolution = decision
          ? previous.pendingResolutions.find(
              (candidate) =>
                candidate.id === decision.continuation.resolutionId,
            )
          : undefined
        if (!decision || !resolution) return state
        const isOptional = decision.type === 'OPTIONAL_EFFECT'
        if (isOptional && selection !== 'YES' && selection !== 'NO')
          return state
        if (
          !isOptional &&
          !decision.options?.some((option) => option.instanceId === selection)
      ...
      finalizeMutationTrace(trace, {
        status: 'MUTATION',
        description: `Resolver decisión ${decisionId}: ${selection}`,
      })
    } catch (error) {
      abortMutationTrace(trace, error)
      throw error
  ...
  replaceGame: (game, options) => {
    set({
      ...
      undoStack: [],
      matchTransactions:
        options?.transactions?.slice(-MAX_RECOVERY_TRANSACTIONS) ?? [],
    })
    const state = get()
    saveMatchRecoverySnapshot(toGameState(state), state.matchTransactions)
  },
...
let recoverySaveTimer: ReturnType<typeof setTimeout> | undefined

const scheduleRecoverySave = (): void => {
...
if (typeof globalThis.window !== 'undefined')
  useGameStore.subscribe(() => scheduleRecoverySave())

/** Applies an action group once, deriving events exclusively from domain state transitions. */
type ActionApplicationMode = 'AUTO' | 'NORMAL' | 'DECLARATION'

type DeclarationApplicationContext = {
  declaration: true
  deferredPending: import('../abilities/types/abilityTypes').PendingAbility[]
  failed: boolean
  fallbackState?: GameState
  rootInitial: GameState
  rootActions: GameAction[]
}

const orderCastDeclarationActions = (actions: GameAction[]): GameAction[] => {
  const castIndex = actions.findIndex((action) => action.type === 'CAST_SPELL')
  if (castIndex <= 0) return actions
  const castAction = actions[castIndex]
  return [
    castAction,
    ...actions.slice(0, castIndex),
    ...actions.slice(castIndex + 1),
  ]
}
...
const applyActionsWithEvents = (
  initial: GameState,
  actions: GameAction[],
  mode: ActionApplicationMode = 'AUTO',
  sharedDeclarationContext?: DeclarationApplicationContext,
): GameState => {
  const isRootApplication = sharedDeclarationContext === undefined
  const declaration =
    sharedDeclarationContext?.declaration === true ||
    mode === 'DECLARATION' ||
    (mode === 'AUTO' && actions.some((action) => action.type === 'CAST_SPELL'))
  const declarationContext: DeclarationApplicationContext | undefined =
    declaration
      ? (sharedDeclarationContext ?? {
          declaration: true,
          deferredPending: [],
          failed: false,
          rootInitial: initial,
  ...
  const appliedActions =
    declaration && isRootApplication
      ? orderCastDeclarationActions(actions)
      : actions
  ...
  const placeOrDeferPending = (
    state: GameState,
    pending: import('../abilities/types/abilityTypes').PendingAbility[],
  ): GameState => {
    if (!pending.length) return state
    if (declarationContext) {
      declarationContext.deferredPending.push(...pending)
      return state
    }
    return addPendingToStack(state, pending)
  }
  const processDerivedEvents = (
    ...
    return placeOrDeferPending(next, pending)
  }
  const addWardTriggersForDeclaration = (
    ...
    return placeOrDeferPending(after, ward)
  }
  ...
  for (
    let actionIndex = 0;
    actionIndex < appliedActions.length;
    actionIndex += 1
  ) {
    const action = appliedActions[actionIndex]
    ...
    if (replacement) {
      ...
      if (replacement.effect.optional) {
        if (declarationContext) {
          const fallback = applyActionsWithEvents(
            declarationContext.rootInitial,
            declarationContext.rootActions,
            'NORMAL',
          )
          declarationContext.fallbackState = fallback
          return fallback
        }
            ...
            decision: {
              ...
              continuation: {
                ...
                replacement: {
                  ...
                  deferredActions: appliedActions.slice(actionIndex + 1),
                },
              },
            },
          },
      ...
      state = applyActionsWithEvents(
        state,
        replacement.replacementActions,
        declarationContext ? 'DECLARATION' : 'NORMAL',
        declarationContext,
      )
      if (declarationContext?.fallbackState)
        return declarationContext.fallbackState
      if (declarationContext?.failed) return declarationContext.rootInitial
      continue
    }
    ...
    if (next === state) {
      if (declarationContext) {
        declarationContext.failed = true
        return declarationContext.rootInitial
      }
      continue
    }
    ...
    if (!declarationContext) state = applySbas(state)
    if (
  ...
  if (isRootApplication && declarationContext) {
    if (declarationContext.fallbackState)
      return declarationContext.fallbackState
    if (declarationContext.failed) return declarationContext.rootInitial
    if (declarationContext.deferredPending.length)
      state = addPendingToStack(state, declarationContext.deferredPending)
